...
function goTo(from, to){
  ...
  // When we enter screen3, place the NO button near YES (after layout exists)
  if (to === screen3) {
...
function openLetter(){
  ...
  // iOS allows audio only after a user gesture; this runs on tap/click
  music.currentTime = 0;
  music.play().catch(()=>{});
...
function placeNoNearYes(){
  ...
  // If screen3 wasn't visible yet, rect can be zeros; guard and retry once
...
yesBtn.addEventListener("click", () => {
  ...
  // Optional: vibration on phones (works if supported)
